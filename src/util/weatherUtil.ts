            ...
            for (const detail of data['weather']) {
                ...
                for (const element of detail['hourly']) {
                    result.push({
                        ...
                        updatedAt: (Math.floor(parseInt(element["time"]) / 100)).toString().padStart(2, '0') + ":00"
                    });
                }
...
export const getEmojiByWeather = (weather: string) => {
    if (!weather) return CircleQuestionMark;

    for (const item of WeatherEmoji) {
