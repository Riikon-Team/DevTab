...
import { editBookmark, fetchBookmarkData } from "@/util/bookmarkUtil"
...
export function EditBookmarkForm() {
    ...
    const formState = useSelector((state: RootState) => state.form.editBookmark)
    ...
    const editBookmarkHandle = () => {
        const result = editBookmark(formState.index, inputName, inputUrl)
        if (result) {
            try {
                dispatcher(updateForm({
                    type: 'editBookmark',
            ...
            <div className="w-full absolute z-1000 flex justify-center items-center " style={{ height: '100vh' }}>
                ...
                <div className="absolute bg-accent p-4 px-6 rounded-2xl w-120 z-1001" onClick={() => { }}>
                    ...
                    <div className="flex flex-col gap-5">
                        <div>
                            <p className="font-semibold text-xl">Edit your bookmark</p>
                            <p className="text-gray-400 mt-2">Type your bookmark URL and name, click Save changes to edit your bookmark.</p>
                        </div>
                        <div>
